...
public class Model: ModelProtocol, CustomStringConvertible, Equatable, RecordStatsDelegate {
    ...
    var createdAt: NSDate {
        let rawDate = self.valueForKey("created_at") as! String
        return AppHelp.convertDate(rawDate)
    }
    ...
    func getCreatedAt() -> NSDate {
        if _created_at == nil {
            let rawDate     = valueForKey("created_at") as! String
    ...
    func idValue(property: String) -> String {
        let value: AnyObject? = valueForKey(property);
    ...
    func removeFromCollection() {
        if let collection = self.collection  {
            collection.removeById(self.idValue("_id"))
        }
    }
    ...
    func valueForKey(key: String) -> AnyObject? {
        let index = data.indexForKey(key);
        if index != nil {
            return data[key]!
        }
    ...
    func hasFormData(json: SwiftyJSON.JSON) -> Bool {
        let formName    = self.formNamed()
        
        if let _ = json[formName].dictionaryObject {
            return true
        }
    ...
    // MARK: - Methods
    func save() -> Request {
        let request = self.api!["create"]!(data: transformData())
    ...
    func fetch() -> Request {
        let data    = ["id": id]
        let request = self.api!["get"]!(data: data)
    ...
    func sync() -> Request {
        ...
        let request = self.api!["update"]!(data: transformData())
        return handlerRequest(request)
    }
    
    func runAction(action: String) -> Request {
        let method = API.method(formNamed(), method: action)
        let data: [String: String] = ["id": id]
        let request = method(data: data)
    ...
    func uploadImage() -> Request {
        ...
        let request = API.uploadImage(self.data)
    ...
    func uploadFile() -> Request {
        performStartSync()
        let request = API.uploadFile(self.data)
    ...
    func remove() -> Request? {
        if !id.isEmpty {
            let request = self.api!["delete"]!(data: ["id": id])
