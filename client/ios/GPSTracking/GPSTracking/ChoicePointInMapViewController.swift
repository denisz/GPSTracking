...
class ChoicePointInMapViewController: UIViewController, XLFormRowDescriptorViewController, GMSMapViewDelegate {
    ...
    func drawUserCircle(coordinate: CLLocationCoordinate2D) {
    ...
    func drawCircle(circleCenter: CLLocationCoordinate2D) {
        if (self.markerCircle != nil) {
            self.markerCircle!.map = nil
        }
        
        self.markerCircle = GMSCircle(position: circleCenter, radius: 1500)
        self.markerCircle!.strokeColor = UIColor(red:0.27, green:0.54, blue:0.95, alpha:1)
        self.markerCircle!.fillColor = UIColor(red:0.27, green:0.54, blue:0.95, alpha:0.2)
        self.markerCircle!.strokeWidth = 1
        self.markerCircle!.map = mapView;
    }
}