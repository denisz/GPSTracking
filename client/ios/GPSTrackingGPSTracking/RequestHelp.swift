...
class RequestHelp {
    class func parseDescription(model: Event) -> [Row] {
        let criteria    = model.valueForKey("criteria") as! [String: AnyObject]
        let context     = criteria["context"] as! String
        let subtype     = criteria["subtype"] as? String
        
        if let extra = criteria["extra"] as? [String: AnyObject] {
    ...
    class func parseView(properties: [String: AnyObject], value: AnyObject) -> Row? {
        var type: String = properties["view"] as! String
    ...
    class func findFieldsForExtra(context: String, subtype: String?) -> [String]? {
        ...
        if let isExtraContext = contextProperties["extra"] as? Bool {
        ...
        if let subtypeValue = subtype {
            ...
            if let isExtraSubtype = subtypeProperties["extra"] as? Bool {
