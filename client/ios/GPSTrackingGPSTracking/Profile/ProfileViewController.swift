...
class ProfileViewController: StatefulViewController, StatefulViewControllerDelegate, MXSegmentedPagerDelegate, MXSegmentedPagerDataSource {
    ...
    func configureView() {
        if let nc = self.navigationController {
            ...
            segmentedPager!.frame = CGRect(origin: CGPoint.zeroPoint, size: self.view.frame.size)
        }
    }
    
    func setupCover() {
        var profileInfoView: ProfileInfoView = ProfileInfoView()
    ...
    func setupCurrentUserControllers() {
        ...
        cancelledRequests.title = "Отмененные"
        ...
        favoritesRequests.title = "Избранное"
