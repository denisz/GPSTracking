...
let SCHEME_SERVER:String = "http";
    #else
let URL_SERVER: String = "192.168.1.5"//"107.170.13.31"
let PORT_SERVER:NSNumber = 3000;
let SCHEME_SERVER:String = "http";
#endif
...
class RestApiRequest {
    ...
    func request() -> NSMutableURLRequest{
        ...
        mutableURLRequest.HTTPBody = NSJSONSerialization.dataWithJSONObject(data, options: nil, error: &JSONSerializationError)
    ...
    func uploadImageRequest() -> (URLRequestConvertible, NSData) {
        var parameters = $.omit(self.data, keys: "image")
...
class ServerRestApi {
    ...
    func downloadFile(request: RestApiRequest) -> Alamofire.Request {
        request.useAccessToken(access_token!)
        
        let destination = Alamofire.Request.suggestedDownloadDestination(directory: .DocumentDirectory, domain: .UserDomainMask)
        
        return Alamofire.download(.GET, request.urlRequest(), destination)
    }
    ...
    func uploadImage(request: RestApiRequest) -> Request {
        ...
        return Alamofire.upload(urlRequest.0, urlRequest.1).debugLog()
    }
